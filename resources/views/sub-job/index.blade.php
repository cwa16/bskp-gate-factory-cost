...
@section('title', 'Sub Job')
@section('page-title', 'Sub Job')
        ...
        <div class="filter-box d-flex justify-content-between align-items-center">
            <h5 class="m-0 fw-bold text-secondary">
                <i class="bi bi-table me-1"></i> Rekap Absensi: <span
                    class="text-primary">{{ \Carbon\Carbon::create($year, $month)->translatedFormat('F Y') }}</span>
            </h5>
            <div class="d-flex gap-2">
                ...
                <form action="{{ route('subjob.index') }}" method="GET" class="d-flex gap-2 mb-0">
            ...
            <table class="table ts-table mb-0" id="mainTable">
                <thead>
                    <tr class="h-row-1">
                        ...
                        <th colspan="{{ $daysInMonth }}" class="sticky-top-th top-1 bg-header">DATE</th>
                    ...
                    <tr class="h-row-2">
                        @for ($d = 1; $d <= $daysInMonth; $d++)
                            ...
                            @endphp
                            <th class="sticky-top-th top-2 {{ $isHoliday ? 'bg-libur' : 'bg-header' }}"
                                title="{{ $title }}" style="width: 25px;">
                                {{ $d }}
                            </th>
                        @endfor
                ...
                <tbody>
                    @foreach ($employees as $index => $emp)
                        @php $totals = $employeeTotals[$emp->nik]; @endphp

                <tbody class="emp-block" data-no="{{ $index + 1 }}" data-name="{{ strtolower($emp->name) }}"
                    data-jabatan="{{ strtolower($emp->jabatan) }}" data-status="{{ strtolower($emp->status) }}"
                    data-ot="{{ $totals['total_ot'] }}" data-rp="{{ $totals['total_rp'] }}"
                    data-brutto="{{ $totals['total_rp'] }}"
                    data-netto="{{ $totals['wages_netto'] ?? $totals['total_rp'] }}">
                    ...
                    <tr class="row-hk">
                        <td rowspan="4" class="sticky-left col-chk" style="background-color: #f8fafc;">
                            <input type="checkbox" class="form-check-input custom-check slip-check border-secondary"
                                data-name="{{ addslashes($emp->name) }}" data-job="{{ $emp->jabatan }}"
                                data-hkrp="{{ $totals['total_hk_rp'] }}" data-otrp="{{ $totals['total_ot_rp'] }}"
                                data-brutto="{{ $totals['total_rp'] }}" data-spsi="{{ $totals['spsi'] ?? 0 }}"
                                data-astek="{{ $totals['astek'] ?? 0 }}" data-listrik="{{ $totals['listrik'] ?? 0 }}"
                                data-kantin="{{ $totals['kantin'] ?? 0 }}"
                                data-spdmotor="{{ $totals['spd_motor'] ?? 0 }}" data-bank="{{ $totals['bank'] ?? 0 }}"
                                data-other="{{ $totals['other'] ?? 0 }}"
                                data-totalpotongan="{{ $totals['total_potongan'] ?? 0 }}"
                                data-netto="{{ $totals['wages_netto'] ?? $totals['total_rp'] }}">
                        </td>
                        <td rowspan="4" class="sticky-left col-no row-number">{{ $index + 1 }}</td>
                        <td rowspan="4" class="sticky-left col-name">
                            <div class="fw-bold text-dark" style="font-size: 11px;">{{ $emp->name }}</div>
                            <div class="text-muted" style="margin-top: 1px;">{{ $emp->nik }} - {{ $emp->status }}
                            </div>
                            <div class="text-muted fw-bold text-primary" style="margin-top: 1px;">{{ $emp->jabatan }}
                            </div>
                        </td>
                        <td class="sticky-left col-type fw-bold text-secondary">HK</td>

                        @for ($d = 1; $d <= $daysInMonth; $d++)
                            @php $cell = $matrix[$emp->nik][$d]; @endphp
                            <td class="{{ $cell['is_holiday'] ? 'bg-libur' : '' }}" title="{{ $cell['holiday_nm'] }}">
                                @if (in_array($cell['desc'], ['H', 'TA', 'L']) || empty($cell['desc']))
                                    {{ $cell['hk_raw'] }}
                                @else
                                    <span class="text-danger fw-bold">{{ $cell['desc'] }}</span>
                                @endif
                            </td>
                        @endfor

                        <td class="fw-bold">{{ $totals['total_hk'] > 0 ? $totals['total_hk'] : '' }}</td>
                        <td></td>
                        <td></td>
                        <td class="text-end pe-2 text-secondary" title="Total Rp Hari Kerja">Rp
                            {{ number_format($totals['total_hk_rp'], 0, ',', '.') }}</td>
                        <td rowspan="4" class="bg-wages-brutto text-end pe-2">Rp
                            {{ number_format($totals['total_rp'], 0, ',', '.') }}</td>
                        <td rowspan="4" class="bg-wages-netto text-end pe-2">
                            {{ number_format($totals['wages_netto'] ?? $totals['total_rp'], 0, ',', '.') }}</td>
                    </tr>
                    ...
                    <tr class="row-ot">
                        ...
                        @for ($d = 1; $d <= $daysInMonth; $d++)
                            @php $cell = $matrix[$emp->nik][$d]; @endphp
                            <td class="{{ $cell['is_holiday'] ? 'bg-libur' : '' }} text-secondary">
                                {{ (float) $cell['ot'] > 0 ? $cell['ot'] : '-' }}
                            </td>
                        @endfor
                        <td></td>
                        <td></td>
                    ...
                    <tr class="row-ot-rev">
                        ...
                        @for ($d = 1; $d <= $daysInMonth; $d++)
                            @php $cell = $matrix[$emp->nik][$d]; @endphp
                            <td class="{{ $cell['is_holiday'] ? 'bg-libur' : '' }} p-0" style="vertical-align: middle;">
                                <input type="number" step="0.5"
                                    class="rev-input {{ $cell['is_holiday'] && $cell['ot_rev'] !== '' ? 'text-red' : '' }}"
                                    value="{{ $cell['ot_rev'] }}" placeholder="-"
                                    onchange="saveRev(this, '{{ $emp->nik }}', '{{ $cell['date'] }}', this.value)">
                            </td>
                        @endfor
                        <td></td>
                        <td></td>
                        <td class="fw-bold text-primary">
                            {{ $totals['total_ot'] > 0 ? number_format($totals['total_ot'], 2, '.', '') : '' }}</td>
                        <td class="text-end pe-2 fw-bold text-primary" title="Total Rp Lembur">
                            {{ $totals['total_ot_rp'] > 0 ? 'Rp ' . number_format($totals['total_ot_rp'], 0, ',', '.') : '' }}
                        </td>
                    </tr>
                    ...
                    <tr class="row-ot-calc">
                        ...
                        @for ($d = 1; $d <= $daysInMonth; $d++)
                            @php $cell = $matrix[$emp->nik][$d]; @endphp
                            <td class="{{ $cell['is_holiday'] ? 'bg-libur' : '' }} text-success fw-bold">
                                {{ (float) $cell['ot_calc'] > 0 ? $cell['ot_calc'] : '-' }}
                            </td>
                        @endfor
                        <td></td>
                        <td></td>
                        ...
                        <td class="fw-bold text-success">
                            {{ $totals['total_ot_final'] > 0 ? number_format($totals['total_ot_final'], 2, '.', '') : '' }}
                        </td>
