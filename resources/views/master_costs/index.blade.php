                            ...
                            @forelse($costs as $index => $item)
                                <tr>
                                    ...
                                    <td class="fw-bold text-success">
                                        Rp {{ number_format($item->cost_per_day, 0, ',', '.') }}
                                    </td>
                                    ...
                                    <td class="text-end pe-4">
                                        <button class="btn btn-sm btn-outline-warning me-1"
                                            onclick="openModal('edit', {{ $item }})" title="Edit">
                                            <i class="bi bi-pencil-square"></i>
                                        </button>

                                        <form action="{{ route('master-costs.destroy', $item->id) }}" method="POST"
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <select name="year" id="input_year" class="form-select" required>
                                ...
                                @for ($y = $currYear - 2; $y <= $currYear + 2; $y++)
    ...
    <script>
        function openModal(mode, data = null) {
            ...
            if (mode === 'create') {
                ...
                form.action = "{{ route('master-costs.store') }}";
            } else if (mode === 'edit') {
                title.innerText = "Edit Master Cost";

                // Set URL Update
                let updateUrl = "{{ route('master-costs.update', ':id') }}";
