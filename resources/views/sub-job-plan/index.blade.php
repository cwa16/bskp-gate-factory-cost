...
@section('content')
    <div class="container-fluid py-3">
        ...
        <div class="card shadow-sm border-0 mb-3">
            <div class="card-body p-2 d-flex align-items-center justify-content-between bg-light">
                <form action="{{ route('subjob.plan.index') }}" method="GET" class="d-flex gap-2 align-items-center">
            ...
            <div class="legend-bar">
                ...
                @foreach ($subJobs as $job)
                    <div class="legend-item" style="background-color: {{ $job->color }};" title="{{ $job->name }}">
                        <span class="legend-id">{{ $job->id }}</span> {{ $job->name }}
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <table class="table-slim">
                <thead>
                    <tr>
                        ...
                        <th colspan="{{ $daysInMonth }}" class="sticky-top-th bg-head-modern" style="top: 0;">
                    ...
                    <tr>
                        @for ($d = 1; $d <= $daysInMonth; $d++)
                            ...
                            <th class="sticky-top-th bg-head-modern {{ $isSun ? 'text-danger bg-light' : '' }}"
                ...
                <tbody>
                    @foreach ($regularEmps as $index => $emp)
                        <tr>
                            <td rowspan="2" class="sticky-col col-no">{{ $index + 1 }}</td>
                            <td rowspan="2" class="sticky-col col-nik fw-bold text-secondary">{{ $emp->nik }}</td>
                            <td rowspan="2" class="sticky-col col-nama text-dark fw-bold">{{ $emp->name }}</td>
                            <td rowspan="2" class="sticky-col col-pos text-muted">{{ $emp->jabatan }}</td>
                            ...
                            @for ($d = 1; $d <= $daysInMonth; $d++)
                                ...
                                @endphp
                                <td style="background-color: {{ $color }}; transition: background 0.2s;">
                                    <input type="number" class="id-input" value="{{ $jobId }}"
                                        data-nik="{{ $emp->nik }}" data-date="{{ $dateStr }}"
                        ...
                        <tr>
                            ...
                            @for ($d = 1; $d <= $daysInMonth; $d++)
                                ...
                                @endphp
                                <td class="{{ $otVal > 0 ? 'bg-danger bg-opacity-10' : '' }}"
                                    style="transition: background 0.2s;">
                                    <input type="number" class="id-input text-danger" value="{{ $otVal }}"
                                        data-nik="{{ $emp->nik }}" data-date="{{ $dateStr }}" step="0.5"
        ...
        <div class="table-responsive-budget">
            <table class="table-slim">
                ...
                <tbody>
                    @foreach ($contractEmps as $index => $emp)
                        <tr>
                            <td rowspan="2" class="sticky-col col-no">{{ $index + 1 }}</td>
                            <td rowspan="2" class="sticky-col col-nik fw-bold text-secondary">{{ $emp->nik }}</td>
                            <td rowspan="2" class="sticky-col col-nama text-dark fw-bold">{{ $emp->name }}</td>
                            ...
                            <div class="mb-2">
                                ...
                                <div class="d-flex gap-1">
                                    <select name="from_month" class="form-select form-select-sm">
                                        @for ($m = 1; $m <= 12; $m++)
                                            <option value="{{ $m }}" {{ $month - 1 == $m ? 'selected' : '' }}>
    ...
    <script>
        ...
        const BATCH_URL = "{{ route('budget-input.save-batch') }}";
